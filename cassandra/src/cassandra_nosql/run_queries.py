...
DEFAULT_OUTPUT_FILE = PROJECT_ROOT / "reports" / "cassandra" / "query_summary.csv"

QUESTION_IDS = {slug: question_id for question_id, slug in QUESTION_SLUGS.items()}
CSV_COLUMNS = ["question_id", "median_time_ms"]

# Each requested timing gets at most this many measured attempts. A failed
# attempt also receives one untimed diagnostic retry before another attempt.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the Cassandra Yelp benchmark. Question IDs follow the "
...
def run_measurements(
    ...
    for question_id in question_ids:
        ...
        for warmup_number in range(1, warmup_runs + 1):
            try:
                execute_query(question_id, session, statements, parameters)
        ...
        median_time_ms = _rounded(statistics.median(timings))
        summaries.append(
            {
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"Cassandra benchmark failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        return 1
    finally:
